...
def entity_head_tag(data):
    POS = {"PUNCT": 0, "ADJ": 1, "CCONJ": 2, "NUM": 3, "DET": 4, "PRON": 5, "ADP": 6, "VERB": 7, "NOUN": 8, "PROPN": 9,
           "ADV": 10, "AUX": 11, "other": 12}
    matrix_heads = np.zeros((len(data)-1, len(POS))) 
    for i in range(len(data)-1):
        e1_search = re.search(r'<e1>[\W\S.,`\':$]+</e1>', data[i])
        e1 = e1_search.group()
...
def sdp(data):
    sdp_matrix = np.zeros((len(data)-1, 1))
    for i in range(len(data)-1):
        data1 = data[i].split("\n")[0]
        e1_search = re.search(r'<e1>[\W\S.,`\':$]+</e1>', data[i])
        e1 = e1_search.group()
...
def emb_after_before(data):
    before_e1_embeding = []
    aft_e2_embeding = []
    for i in range(len(data)-1):
        data1 = data[i].split("\n")[0]
        before_e1 = re.search(r'[\W\S.,`\':$<>/]+<e1>', data1).group()
        ...
        before_e1_embeding.append(word_emb_bef_e1)


        after_e2 = re.search(r'</e2>[\W\S.,`\':$<>/]+', data1).group()
        ...
        aft_e2_embeding.append(word_emb_aft_e2)

    return np.array(before_e1_embeding),np.array(aft_e2_embeding)
...
def get_features(file_name, relations_list):
    ...
    POS_head = entity_head_tag(splited_data)
    sdp_e1_e2 = sdp(splited_data)
    Wbef_e1,Waft_e2 = emb_after_before(splited_data)

    features = np.concatenate((e1Type, e2Type,e1e2distance, prepositions, e1_emb,
                e2_emb,word_emb_e1_e2,POS_beetween_e1e2), axis=1)
